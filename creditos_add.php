...
<script language="javascript">
   ...
   var montocancelar = GuardarVenta.montopagar1.value;  //total de la venta
   var montorecibido=GuardarVenta.numerotc.value;
   var tipocredito=GuardarVenta.comprobante.value;   
   var ncuota=GuardarVenta.cuota.value;   //cantidad de cuta
   var mpago=GuardarVenta.metodopago.value;   //mensual men, quincenal qui, semanal sem
   var mtea=GuardarVenta.tea.value;   //mensual men, quincenal qui, semanal sem
   if(montorecibido=='')
      montorecibido=0;
    if(tipocredito=="csi")
      montorecibido=0;
   capitalf=(parseFloat(montocancelar)-parseFloat(montorecibido)).toFixed(2);//monto de credito
   
   interesanual=((parseFloat(capitalf)*parseFloat(mtea))/100).toFixed(2);

  // if(mpago=="men")
  //    interescredito=(parseFloat(interesanual)/12).toFixed(2);
  // if(mpago=="qui")
  //    interescredito=(parseFloat(interesanual)/24).toFixed(2);
  // if(mpago=="sem")
  //    interescredito=(parseFloat(interesanual)/51).toFixed(2);
    //interescredito=interescredito*ncuota;
  // montocredito=(parseFloat(capitalf)+parseFloat(interescredito)).toFixed(2);
   montocredito=(parseFloat(capitalf)+parseFloat(interesanual)).toFixed(2);
   cuotamespago=(parseFloat(montocredito)/ncuota).toFixed(2);

/*		var monto = GuardarVenta.numerotc1.value;
	...
	*/
		...
		document.getElementsByName("capital")[0].value = capitalf;
    document.getElementsByName("totalcredito")[0].value = montocredito;
    document.getElementsByName("cuotames")[0].value = cuotamespago;
	/*	if(vuelto>=0){
...
if ((isset($_POST["MM_GuardarVenta"])) && ($_POST["MM_GuardarVenta"] == "GuardarVenta")) {	
  ...
  $insertSQL = sprintf("insert into credito values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s)",
              GetSQLValueString(NULL, "text"),
						  ...
						  GetSQLValueString($_POST['montopagar1'], "double"),
						  ...
						  GetSQLValueString($_POST['numerotc'], "double"),
		          GetSQLValueString($_POST['montopagar1']-$_POST['numerotc'], "double"),
              GetSQLValueString(1, "int"),
              GetSQLValueString($_POST['metodopago'], "text"),
              GetSQLValueString($_POST['cuota'], "double"),
              GetSQLValueString($_POST['tea'], "double"),
              GetSQLValueString($_POST['dia_pago'], "int"),
              GetSQLValueString($_POST['capital'], "float"),
              GetSQLValueString($_POST['totalcredito'], "float"),
              GetSQLValueString($_POST['cuotames'], "double"),
...
$updateSQL = sprintf("UPDATE producto_stock INNER JOIN detalle_ventas ON producto_stock.codigoprod = detalle_ventas.codigoprod SET producto_stock.stock = producto_stock.stock-detalle_ventas.cantidad where detalle_ventas.codcomprobante=%s",
  ...
  $rs = mysql_query("SELECT MAX(codigocredito) AS id FROM credito");
if ($row = mysql_fetch_row($rs)) {
$id = trim($row[0]);
}

if($_POST['metodopago']=="men"){

  for($i=1; $i<=$_POST['cuota']; $i++)
  {
        $nuevafecha = strtotime ( "+$i month" , strtotime ( $fechapago) ) ;
        $nuevafecha = date ( "Y-m-$_POST[dia_pago]" , $nuevafecha );
    
        $insertSQL = sprintf("insert into cronogramacredito values (%s, %s, %s, %s, %s, %s, %s)",
              GetSQLValueString(NULL, "text"),
              GetSQLValueString($i, "int"),
              GetSQLValueString($_POST['cuotames'], "int"),
              GetSQLValueString(0, "text"),
              GetSQLValueString($nuevafecha, "text"),
              GetSQLValueString(round($_POST['totalcredito']-$_POST['cuotames']*$i,2), "text"),
...
if($_POST['metodopago']=="qui"){
  $cuotax=$_POST['dia_pago'];
  for($i=1; $i<=$_POST['cuota']; $i++)
  {     

        $nuevafecha = strtotime ( "+$cuotax day" , strtotime ( $fechapago) ) ;
...
?><script type="text/javascript">
window.location="creditos_list.php";
</script>
...
$totalRows_Contador_Clientes = mysql_num_rows($Contador_Clientes);
  ...
  $insertGoTo = "creditos_add.php?codigo=" . $_GET['codigo'] . "";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<select id="single-append-text" class="form-control select2-allow-clear" name="codigoprod">
  <option value="" <?php if (!(strcmp("", "creditos_add.php"))) {echo "selected=\"selected\"";} ?>></option>
  <?php
do {  
?>
  <option value="<?php echo $row_Productos['codigoprod']?>"<?php if (!(strcmp($row_Productos['codigoprod'], "creditos_add.php"))) {echo "selected=\"selected\"";} ?>><?php echo $row_Productos['nombre_producto']?> - <?php echo $row_Productos['Marca']; ?> - <?php echo $row_Productos['nombre_color']; ?> - <?php echo "$/.". $row_Productos['precio_venta']; ?> (<?php echo "Stock ".$row_Productos['stock']; ?>)</option>
  <?php
 ...
 <tbody>
        <?php do { ?>
          <tr>
            ...
            <input name="pventa" type="text" class="form-control tooltips input-sm"  id="pventa" value="<?php echo $row_Detalle_Ventas['pventa']; ?>"  maxlength="7" data-placement="top" data-original-title="<?php echo $row_Detalle_Ventas['precio_compra']; ?>"/>
...
<select name="comprobante" id="comprobante" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Comprobante" required OnChange="formapagoOnChange(this)">
  
  <option value="cre">Credito con Inicial</option>
  <option value="csi">Credito sin Inicial</option>
...
<script type="text/javascript">
function formapagoOnChange(sel) {
  
      if (sel.value=="cre"){
           divC = document.getElementById("tipopago");
           divC.style.display = "";
 
           divT = document.getElementById("numerotc");
           divT.style.display = "";
           ...
           divT = document.getElementById("montorecibido");
           divT.style.display = "none";
      }
}
</script>
...
<select name="tipopago" id="tipopago" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar forma de Pago" required onChange="pagoOnChange(this)">
...
<td><span id="sprytextfield3">
  <input  name="numerotc" type="text" class="form-control tooltips "  id="numerotc" value=""  maxlength="6" data-placement="top" data-original-title="Monto Recibido" />
...
<div class="col-md-13">
<div class="form-group"><span id="spryselect2">
<select name="metodopago" id="metodopago" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Forma de Pago" required >
...
<span id="sprytextfield3">
  <input  name="cuota" type="text" class="form-control tooltips "  id="cuota" value=""  maxlength="6" data-placement="top" data-original-title="Numero de Cuotas" onblur="calcularvuelto()" onblur="mensaje()"   required="required"/>
  <span class="textfieldRequiredMsg"></span></span>
...
<div class="col-md-13">
<div class="form-group"><span id="spryselect2">
<select name="tea" id="tea" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Forma de Pago" required  onclick="calcularvuelto()" onclick="mensaje()"  >
  <option value=0>0% TEA</option>
  <option value=10>10% TEA</option>
  <option value=20>20% TEA</option>
  <option value=30>30% TEA</option>
  <option value=40>40% TEA</option>
  <option value=50>50% TEA</option>
  <option value=75>75% TEA</option>
  <option value=100>100% TEA</option>
  <option value=120>120% TEA</option>
...
<div class="col-md-13">
<div class="form-group"><span id="spryselect2">
<select name="dia_pago" id="dia_pago" class="form-control tooltips" data-placement="top" data-original-title="Dia de Pago" required>
...
<span id="sprytextfield1">
  ...
  <input  name="capital" type="text" class="form-control tooltips "  id="capital" value=""  maxlength="50" data-placement="top" data-original-title="Capital a Financiar"   readonly="readonly"  />
  <span class="textfieldRequiredMsg"></span></span>
  </div>
</td>
<td valign="top">
...
<span id="sprytextfield1">
  ...
  <input  name="totalcredito" type="text" class="form-control tooltips "  id="totalcredito" value=""  maxlength="50" data-placement="top" data-original-title="Total de credito"   readonly="readonly" />
  <span class="textfieldRequiredMsg"></span></span>
  </div>
  </td>
</tr>
...
<span id="sprytextfield1">
  ...
  <input  name="cuotames" type="text" class="form-control tooltips "  id="cuotames" value=""  maxlength="50" data-placement="top" data-original-title="Cuota Mensual"   readonly="readonly"  />
  <span class="textfieldRequiredMsg"></span></span>
  </div>
</td>
</tr>
...
<input type="hidden" name="totalc" id="totalc" align="center"  readonly="readonly" value="<?php echo number_format($totalc,2);?>"  size="8" hidden="true"  /></h5></td>
</tr>
<tr>
<td colspan="2">
