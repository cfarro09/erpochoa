					...
					<div class="container-fluid">
						<div class="row">
							<div class="col-xs-12 col-md-12">
								...
								DOC REF 1 : <span id="mcodref11"></span> <br>
								...
								<div class="row" style="margin-top: 20px">
									<div class="col-xs-12 col-md-12">
										<div class="row">
											...
											<div class="col-md-4">
												<div class="form-group">
													<label for="field-1" class="control-label">Tipo de
														Comprobante</label>
													<select class="form-control select2-allow-clear"
														name="tipocomprobante" id="tipocomprobante">
...
<script type="text/javascript">
		...
		item.addEventListener("click", e => {
			...
			fetch(url)
				...
				.then(res => {
					...
					$("#mcodref11").text(res.header.codigoref1)
					...
					res.detalle.forEach(r => {
						...
						$("#detalleFacturar-list").append(`
							...
							<td >${r.marca}</td>
							<td><input data-toggle="tooltip" data-placement="bottom" title="0" oninput="changepreciocompra(this)" value="${r.pcompra}" required type="number" class="precio-compra form-control"></td>
							
							<td class="importe">${r.pcompra ? (r.pcompra * r.cantidad).toFixed(4) : ""}</td>
							</tr>`);
						let suma = 0;
						getSelectorAll(".importe").forEach(item => {
							if (item.textContent)
								suma += parseFloat(item.textContent)
						})
	...
	function changepreciocompra(e, aux = true) {
		...
		if (aux) {
			aa.querySelector(".importe").textContent = parseFloat(ss).toFixed(4)
			let total = 0;
			let subtotal = 0;
			document.querySelectorAll(".importe").forEach(item => {
				if (item.textContent) {
					total += item.textContent * 1.18;
	...
	document.querySelector("#saveFacturar").addEventListener("submit", e => {
		...
		let sumaventa = 0;
		// getSelectorAll(".precio-venta").forEach(item => {
		// 	const cantidad = parseInt(item.closest('tr').querySelector(".cantidad").textContent)
		// 	sumaventa += item.value*cantidad
		// })
		data.header = {
			codigocompras: 0,
			tipocomprobante: getSelector("#tipocomprobante").value,
		...
		getSelectorAll("#detalleFacturar-list tr").forEach(item => {
			data.detalle.push({
				...
				pventa: 0,
				pcompra: parseFloat(item.querySelector(".precio-compra").value).toFixed(4),
				igv: parseFloat(parseFloat(item.querySelector(".precio-compra").value) * 0.18).toFixed(4),
				totalcompras: parseFloat(parseFloat(item.querySelector(".precio-compra").value) * item.querySelector(".cantidad").textContent).toFixed(4)
			})
		})
		console.log(data)
		var formData = new FormData();
