...
if ((isset($_POST["MM_GuardarVenta"])) && ($_POST["MM_GuardarVenta"] == "GuardarVenta")) {	
     $codigocomp=$_POST['comprobante'].$_POST['numcomprobante'];
  ...
  $insertSQL = sprintf("insert into ventas values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
						  ...
						  GetSQLValueString($_POST['numcomprobante'], "text"),
						  ...
						  GetSQLValueString($_POST['igv'], "text"),
...
$query_Productos = "SELECT codigoprod, precio_venta FROM historial_producto WHERE codigoprod = $codigoproducto10 ORDER BY codigohistprod desc";
...
$totalRows_Contador_Clientes = mysql_num_rows($Contador_Clientes);
  ...
  $insertGoTo = "ventas_add.php?codigo=" . $_GET['codigo'] . "";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_Ventas, $Ventas);
$query_Productos = "SELECT * FROM vt_producto where stock>=1";
$Productos = mysql_query($query_Productos, $Ventas) or die(mysql_error());
$row_Productos = mysql_fetch_assoc($Productos);
$totalRows_Productos = mysql_num_rows($Productos);
...
mysql_select_db($database_Ventas, $Ventas);
$query_Detalle_Ventas = sprintf("SELECT a.codigodetalleproducto, a.codigo, a.codigoprod, a.cantidad,b.nombre_producto AS Producto, c.nombre AS Marca, d.precio_venta, e.nombre_color, sum(a.cantidad*d.precio_venta) as Importe, ps.stock, d.precio_compra, a.pventa  FROM detalle_ventas a  INNER JOIN producto b ON a.codigoprod =b.codigoprod INNER JOIN marca c ON b.codigomarca = c.codigomarca INNER JOIN historial_producto d ON a.codigoprod = d.codigoprod INNER JOIN color e ON b.codigocolor = e.codigocolor INNER JOIN producto_stock ps ON ps.codigoprod=a.codigoprod WHERE a.codigo = %s  AND d.precio_compra>0 GROUP BY a.codigoprod  desc ", GetSQLValueString($colname_Detalle_Ventas, "text"));
...
mysql_select_db($database_Ventas, $Ventas);
$query_Clientes = "SELECT codigoclienten, CONCAT(paterno,  ' ', materno, ' ', nombre) as ClienteNatural FROM cnatural  WHERE estado = 0";
 ...
 <tbody>
        <?php do { ?>
          <tr>
            ...
            <input name="pventa" type="text" class="form-control tooltips input-sm"  id="pventa" value="<?php echo $row_Detalle_Ventas['pventa']; ?>"  maxlength="3" data-placement="top" data-original-title="<?php echo $row_Detalle_Ventas['precio_compra']; ?>"/>
...
function TClienteOnChange(sel) {
      if (sel.value=="fac"){
           ...
           divC.style.display = "";
 		   divT = document.getElementById("ClienteN");
...
<select name="comprobante" id="comprobante" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Comprobante" required OnChange="TClienteOnChange(this)">
  ...
  <option value="tic" disabled="disabled">Ticket</option>
...
<select name="tipopago" id="tipopago" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Comprobante" required onChange="pagoOnChange(this)">
  <option value="">- Tipo Pago -</option>
    <option value="p_c">Pago Contado</option>
...
<span id="sprytextfield2">
  <input name="numcomprobante" type="text" class="form-control tooltips "  id="numcomprobante" value=""  maxlength="16" data-placement="top" data-original-title="Numero Comprobante" required/>
  <span class="textfieldRequiredMsg"></span></span>
</td>
<td><span id="sprytextfield3">
  <input  name="numerotc" type="text" class="form-control tooltips "  id="numerotc" value=""  maxlength="6" data-placement="top" data-original-title="Monto Recibido"  required="required"/>
...
<td><span id="sprytextfield3">
  <input  name="cambio" type="text" class="form-control tooltips "  id="cambio" value="<?php 
  if ($_POST['numerotc']==NULL)
  echo("");
  else
  echo($_POST['numerotc']);?>"  maxlength="6" data-placement="top" data-original-title="Vuelto o Cambio"  required="required" disabled="disabled"/>
  <span class="textfieldRequiredMsg"></span></span></td>
</tr>
<br />
<tr>
...
<div class="form-group" id="ClienteJ" style="display:none"><span id="spryselect6">
  <select name="codigoclientej" class="form-control select2 tooltips" id="single" data-placement="top" data-original-title="Seleccionar Comprobante" >
    <optgroup label="Clientes Jur&iacute;dico"> </optgroup>
    <option value=""></option>
    <?php
do {  
?>
    <option value="<?php echo $row_ClientesJuridicos['codigoclientej']?>"><?php echo $row_ClientesJuridicos['razonsocial']?></option>
    <?php
...
<td valign="top" align="right">
<h5 class="font-red-mint"><strong>$. <?php echo round($subtotal,2); ?><input type="text" name="subtotal" id="subtotal" align="center"  readonly="readonly" value="<?php echo round($subtotal,2);?>"  size="8" hidden="true"  /></strong></h5></td>
</tr>
<tr>
<td valign="top" align="right">
<h5 class="font-red-mint"><strong>IVA 14%</strong></h5>
</td>
<td valign="top"align="right"><h5 class="font-red-mint"><strong>$. <?php echo round($iva,2);?> 
<input type="text" name="igv" id="igv" align="center"  readonly="readonly" value="<?php echo round($iva,2);?>"  size="8" hidden="true"  /></strong></h5></td>
</tr>
<tr>
<td valign="top" align="right">
<h5 class="font-red-mint"><strong>TOTAL $.</strong></h5></td>
<td valign="top" align="right">
<h5 class="font-red-mint"><strong><?php echo round($total,2);?><input type="text" name="total" id="total" align="center"  readonly="readonly" value="<?php echo round($total,2);?>"  size="8" hidden="true"  /></h5></td>
</tr>
<tr>
<td colspan="2">
