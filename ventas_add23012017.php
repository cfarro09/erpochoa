...
if ((isset($_POST["MM_GuardarVenta"])) && ($_POST["MM_GuardarVenta"] == "GuardarVenta")) {	
 ...
 $insertSQL = sprintf("insert into ventas values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
						  ...
						  GetSQLValueString($_POST['igv'], "float"),
...
window.location="ventas_list.php";
</script>
...
$query_Productos = "SELECT codigoprod, precio_venta FROM producto_stock WHERE codigoprod = $codigoproducto10 ORDER BY codigoprod desc";
...
$totalRows_Contador_Clientes = mysql_num_rows($Contador_Clientes);
  ...
  header("Location: www.compraenunclic.com/ventas/ventas_list.php");
}
//actualiza stock
...
mysql_select_db($database_Ventas, $Ventas);
$query_Detalle_Ventas = sprintf("SELECT a.codigodetalleproducto, a.codigo, a.codigoprod, a.cantidad,b.nombre_producto AS Producto, c.nombre AS Marca, ps.precio_venta, e.nombre_color, sum(a.cantidad*ps.precio_venta) as Importe, ps.stock, ps.precio_compra, a.pventa  FROM detalle_ventas a  INNER JOIN producto b ON a.codigoprod =b.codigoprod INNER JOIN marca c ON b.codigomarca = c.codigomarca INNER JOIN historial_producto d ON a.codigoprod = d.codigoprod INNER JOIN color e ON b.codigocolor = e.codigocolor INNER JOIN producto_stock ps ON ps.codigoprod=a.codigoprod WHERE a.codigo = %s  AND d.precio_compra>0 GROUP BY a.codigoprod desc ", GetSQLValueString($colname_Detalle_Ventas, "text"));
...
<td valign="top" align="right">
<h5 class="font-red-mint"><strong>S/. <?php echo number_format($subtotal,2); ?><input type="text" name="subtotal" id="subtotal" align="center"  readonly="readonly" value="<?php echo number_format($subtotal,2);?>"  size="8" hidden="true"  /></strong></h5></td>
</tr>
<tr>
<td valign="top" align="right">
<h5 class="font-red-mint"><strong>IVA 18%</strong></h5>
</td>
<td valign="top"align="right"><h5 class="font-red-mint"><strong>S/. <?php echo number_format($iva,2);?> 
...
<tr>
<td valign="top" align="right">
<h5 class="font-red-mint"><strong>TOTAL S/.</strong></h5></td>
<td valign="top" align="right">
