									...
									<div class="col-md-12">
										<div class="form-group">
											...
											<select name="codigosuc" required id="codigosuc" class="sucursalXX form-control select2 tooltips" id="single" data-placement="top">
												...
												do {
												?>
													<option <?= $row_sucursales['cod_sucursal'] == $codsucursal ? "selected" : "" ?> value="<?= $row_sucursales['cod_sucursal'] ?>"><?= $row_sucursales['nombre_sucursal'] ?></option>
												<?php
...
<script>
	...
	const searchdetail = () => {
		...
		fetch(`getKardexContableFromProductList.php`, {
				method: 'POST',
				body: formData
			})
			.then(res => res.json())
				...
				if (res.length > 0) {
					...
					res.forEach(item => {
						...
						if (item.cantidad != "0") {
							...
							<td>${item.numero}</td>
							<td>${item.precio}</td>
							<td>${(item.detalle.includes("Compras") || (item.detalle.includes("Ventas") && item.tipocomprobante == "notacredito")) || item.detalle.includes("Entra") || item.detalle.includes("anulacion") ? item.cantidad : ""}</td>
							<td>${(item.detalle.includes("Compras") || (item.detalle.includes("Ventas") && item.tipocomprobante == "notacredito")) || item.detalle.includes("Entra") || item.detalle.includes("anulacion") ? item.preciototal : ""}</td>
	...
	const initTable = async () => {
        ...
        left join precio_venta pv on pv.codigo_pv = (select max(pv1.codigo_pv) from precio_venta pv1 where pv1.codigoprod = p.codigoprod)
        inner join kardex_contable k on k.codigoprod = p.codigoprod and k.id_kardex_contable = (select max(k1.id_kardex_contable) from kardex_contable k1 where k1.codigoprod = k.codigoprod)
		where k.sucursal = ${codsucursal}
        `
