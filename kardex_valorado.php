			...
			<?php if ($totalRows_personal > 0): ?>
				<table class="table table-bordered table-hover" id="sample_1">
					...
					<tr>
							...
							<th> </th>
							<th>  </th>
							<th style="background: #32c5d2; color: white" class="text-center" colspan="2">ENTRADAS</th>
							<th style="background: #32c5d2; color: white" class="text-center" colspan="2">SALIDAS</th>
							<th style="background: #32c5d2; color: white" class="text-center" colspan="2">SALDO</th>
						</tr>
						<tr>
							<th class="text-center"> Produco </th>
					...
					<tbody>
						...
						$query_kardex = "select k.id_kardex_contable, s.nombre_sucursal, p.nombre_producto , k.codigoprod, k.fecha, k.numero, k.detalle, k.cantidad, k.precio, k.saldo from kardex_contable k left join producto p on p.codigoprod = k.codigoprod left join sucursal s on s.cod_sucursal = k.sucursal
						";
						$listado_kardex = mysql_query($query_kardex, $Ventas) or die(mysql_error());
						$row_kardex = mysql_fetch_assoc($listado_kardex);
						?>
						<?php do {  ?>
							<tr>
								<td class="text-center"><?= $row_kardex['nombre_producto'] ?></td>
								<td class="text-center"><?= $row_kardex['fecha'] ?></td>
								<td class="text-center"><?= $row_kardex['precio'] ?></td>
								<td class="text-center"><?= $row_kardex['cantidad'] ?></td>
								<td class="text-center"><?= "S/ ". number_format($row_kardex['cantidad']*$row_kardex['precio'], 4) ?></td>
								<td class="text-center"></td>
								<td class="text-center"></td>
								<td class="text-center"><?= $row_kardex['saldo'] ?></td>
								<td class="text-center"><?= "S/ ". number_format($row_kardex['saldo']*$row_kardex['precio'], 4) ?></td>
							</tr>
								<?php $i++;} while ($row_kardex = mysql_fetch_assoc($listado_kardex)); ?>
							</tbody>
						</table>
					<?php endif ?>
				</div>
