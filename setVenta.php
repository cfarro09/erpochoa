...
if($queryheader){
	mysql_query($queryheader, $Ventas) or die(mysql_error() . "  " . $queryheader);
	$lastId = mysql_query("SELECT LAST_INSERT_ID()", $Ventas) or die(mysql_error(). "  ". $queryheader);
...
foreach ($detalleArray as $querydetalle) {
	...
	mysql_query($querydetalle, $Ventas) or die(mysql_error() . "  " . $querydetalle);
}
