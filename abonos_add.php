...
if ((isset($_POST["MM_GuardarVenta"])) && ($_POST["MM_GuardarVenta"] == "GuardarVenta")) {	
    ...
    if($_POST['codigoclienten']==NULL || $_POST['montofact']==NULL)
    { ?>    
  ...
  */
  ...
  $insertSQL = sprintf("insert into abonos values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                          GetSQLValueString(NULL, "text"),
						  GetSQLValueString($codigocomp, "text"),
						  GetSQLValueString($_POST['codigobanco'], "int"),
						  GetSQLValueString($_POST['numerotarjeta'], "text"),
		          ...
		          GetSQLValueString($_POST['montofact']-$_POST['numerotc'], "double"),
              GetSQLValueString(2, "int"),
              GetSQLValueString($_POST['totalc'], "double"));

    $insertSQL1 = sprintf("insert into abonosaldo values (%s, %s, %s)",
              GetSQLValueString(NULL, "text"),
              GetSQLValueString($codigocomp, "text"),
              GetSQLValueString($_POST['montofact']-$_POST['numerotc'], "double"));

  mysql_select_db($database_Ventas, $Ventas);
  $Result2 = mysql_query($insertSQL1, $Ventas) or die(mysql_error());
...
?><script type="text/javascript">
window.location="abonos_list.php";
</script>
...
$totalRows_Contador_Clientes = mysql_num_rows($Contador_Clientes);
  ...
  $insertSQL = sprintf("INSERT INTO detalle_ventas (codigo, codigoprod, pventa, concatenacion, pcompra) VALUES (%s, %s, %s, %s, %s)",
					   ...
					   GetSQLValueString($concatenacion,"text"), GetSQLValueString($row_Productos['precio_compra'], "double"));
  ...
  $insertGoTo = "abonos_add.php?codigo=" . $_GET['codigo'] . "";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<select id="single-append-text" class="form-control select2-allow-clear" name="codigoprod">
  <option value="" <?php if (!(strcmp("", "abonos_add.php"))) {echo "selected=\"selected\"";} ?>></option>
  <?php
do {  
?>
  <option value="<?php echo $row_Productos['codigoprod']?>"<?php if (!(strcmp($row_Productos['codigoprod'], "abonos_add.php"))) {echo "selected=\"selected\"";} ?>><?php echo $row_Productos['nombre_producto']?> - <?php echo $row_Productos['Marca']; ?> - <?php echo $row_Productos['nombre_color']; ?> - <?php echo "$/.". $row_Productos['precio_venta']; ?> (<?php echo "Stock ".$row_Productos['stock']; ?>)</option>
  <?php
