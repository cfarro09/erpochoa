...
$query_Productos = "select a.codigoprod , a.nombre_producto, b.codigomarca ,b.nombre AS Marca,
c.nombre_color, pres.nombre_presentacion AS umedida, a.minicodigo
from producto a 
...
<script type="text/javascript">
	const ordencompraid = '<?=  $ordecompraid ?>';
	...
	const oninit = async () => {
		const queryheader = `select * from ordencompra where codigo = '${ordencompraid}'`
		const resheader = await get_data_dynamic(queryheader);
		
		const header = resheader[0];
		console.log(header);
		
		$('#sucursal-oc-new').val(header.sucursal).trigger('change');
		$('#proveedor').val(header.codigoproveedor).trigger('change');
		$("#codigoreferencial1").val(header.codigoref1);
		$("#codigoreferencia2").val(header.codigoref2);

		getSelector("#subtotal-header").textContent = header.subtotal;
		getSelector("#total-header").textContent = header.montofact;
		getSelector("#igv-header").textContent = header.igv;

		const querydetail = `
		select ddd.*, a.codigoprod , a.nombre_producto, b.codigomarca ,b.nombre AS Marca,
		c.nombre_color, pres.nombre_presentacion AS umedida, a.minicodigo 
		from detalle_compras_oc ddd  
		inner join producto a on a.codigoprod = ddd.codigoprod 
		join marca b on a.codigomarca = b.codigomarca 
		join color c on a.codigocolor = c.codigocolor 
		join presentacion pres on pres.codigopresent = a.codigopresent
		where ddd.codigo = '${ordencompraid}'`
		let resdetail = await get_data_dynamic(querydetail);
		...
		resdetail.forEach(x => {
			...
			$("#detalleFormProducto").append(`
					<tr class="producto">
					<td data-codigo="${x.codigoprod}" class="codigopro codigo_${x.codigoprod}" style="display: none">${x.codigoprod}</td>
					<td class="indexproducto">${cantrows}</td>
					<td><input type="text" oninput="changevalue(this)" value="${x.cantidad}" required class="cantidad form-control" value="" style="width: 80px" ></td>
					...
					<td class="nombre">${x.nombre_producto}</td>
					<td style="display: none" class="codigodetalle">${parseInt(x.codigodetalleproducto)}</td>
					<td class="marca">${x.Marca}</td>
					<td style="width: 100px"><input value="${x.pcompra}" type="number" oninput="changevalue(this)" required class="precio form-control" value="0" ></td>
					<td class="importe" value="${x.pcompra * x.cantidad}" >0</td>
					<td>
	...
	const triggerchange = () => {
		if (!getSelector(`.codigo_${codigoprod.value}`)) {
			
			const option = codigoprod.options[codigoprod.selectedIndex]
			const cantrows = document.querySelectorAll("#detalleFormProducto tr").length + 1
			$("#detalleFormProducto").append(`
					<tr class="producto">
					<td data-codigo="${codigoprod.value}" class="codigopro codigo_${codigoprod.value}" style="display: none">${this.value}</td>
					<td class="indexproducto">${cantrows}</td>
		...
		if (getSelectorAll(".producto").length < 1) {
			...
			getSelectorAll(".producto").forEach(item => {
				data.detalle.push({
					...
					codigodetalle: item.querySelector(".codigodetalle") ? "ACTUALIZAR" : "INSERTAR",
