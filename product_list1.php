...
mysql_select_db($database_Ventas, $Ventas);
$query_Listado = "SELECT a.codigoprod, a.nombre_producto, b.nombre AS Marca, c.nombre AS Categoria, d.nombre AS SubCategoria, e.nombre_color AS Color, f.nombre_presentacion AS Presentacion, max(h.precio_compra) as precio_compra, max(h.precio_venta) as precio_venta, sum(h.cantidad) as cantidad, h.codigoproveedor, ps.stock as Stock FROM producto a INNER JOIN marca b ON a.codigomarca = b.codigomarca INNER JOIN categoria c ON a.codigocat = c.codigocat INNER JOIN subcategoria d ON a.codigosubcat = d.codigosubcat INNER JOIN color e ON a.codigocolor = e.codigocolor INNER JOIN presentacion f ON a.codigopresent = f.codigopresent INNER JOIN historial_producto h ON h.codigoprod=a.codigoprod INNER JOIN producto_stock ps ON ps.codigoprod=a.codigoprod WHERE a.estado = 0 group by a.codigoprod order by codigoprod";
    ...
    <tbody>
      <?php do { ?>
        <tr>
          ...
          <td> 
          <a class="btn yellow-crusta tooltips" data-placement="top" data-original-title="Stock" onClick="abre_ventana('Emergentes/product_list_cantidad.php?codigoprod=<?php echo $row_Listado['codigoprod']; ?>',<?php echo $popupAncho?>,<?php echo $popupAlto?>)"><i class="glyphicon glyphicon-credit-card" ></i></a>          </td>
            ...
            <td> 
            <a  class="btn blue-ebonyclay tooltips" data-placement="top" data-original-title="Actualizar Ultima Compra"  onClick="abre_ventana('Emergentes/product_list_editar_cantidad.php?codigoprod=<?php echo $row_Listado['codigoprod']; ?>',<?php echo $popupAncho?>,<?php echo $popupAlto?>)"><i class="fa fa-refresh" ></i></a>  
            </td>
            <td> 
