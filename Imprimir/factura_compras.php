...
$query_Factura = sprintf("SELECT c.codigocompras, c.subtotal, c.igv, c.montofact, c.fecha_emision, c.codigoproveedor, c.codigocompra, c.numerofactura, pe.nombre as nombrep, c.fecha_emision, pe.paterno as paternop, pe.materno as maternop, p.celular, p.ciudad, p.direccion, p.email, p.pais, p.paginaweb, p.telefono, p.ruc, p.razonsocial FROM compras c inner join acceso a on a.codacceso=c.codacceso inner join personal pe on pe.codigopersonal=c.codigopersonal inner join proveedor p on p.codigoproveedor=c.codigoproveedor WHERE c.codigocompras = %s", GetSQLValueString($colname_Factura, "int"));
...
mysql_select_db($database_Ventas, $Ventas);
$query_Listado_Productos = "SELECT a.codigodetalleproducto, a.codigo, a.codigoprod, (a.cantidad*a.pcompra) AS total, a.cantidad, a.pcompra, a.concatenacion, a.codcomprobante, b.nombre_producto AS Producto, c.nombre AS Marca, d.nombre_presentacion AS Presentacion, e.nombre_color AS Color FROM detalle_compras a INNER JOIN producto b ON a.codigoprod = b.codigoprod INNER JOIN marca c ON b.codigomarca = c.codigomarca INNER JOIN presentacion d ON b.codigopresent = d.codigopresent INNER JOIN color e ON b.codigocolor = e.codigocolor WHERE a.codcomprobante = '$colname_Listado_Productos'";
...
$pdf->Cell(0,15,'                                    FACTURA DE COMPRA:  '.$row_Factura['codigocompras'],0,1);
...
$pdf->SetFont('Times','',9);
	...
	$pdf->Cell(0,5,'FACTURA N.       :   '.$row_Factura['numerofactura'],0,1);
