...
if (isset($_GET['codigocompras'])) {
  $colname_Factura = $_GET['codigocompras'];
	$colname_codigop = $_GET['codigop'];  
  $colname_Listado_Productos = $_GET['codigocompras'];
}
mysql_select_db($database_Ventas, $Ventas);

$query_Factura = sprintf("SELECT c.codigoordcomp, c.codigo, c.subtotal, c.igv, c.montofact, c.fecha_emision, c.codigoproveedor, c.codigo, c.codigoref1, c.codigoref2, pe.nombre as nombrep, c.fecha_emision, pe.paterno as paternop, pe.materno as maternop, p.celular, p.ciudad, p.direccion, p.email, p.pais, p.paginaweb, p.telefono, p.ruc, p.razonsocial FROM ordencompra c inner join acceso a on a.codacceso=c.codacceso inner join personal pe on pe.codigopersonal=c.codigopersonal inner join proveedor p on p.codigoproveedor=c.codigoproveedor WHERE p.codigoproveedor = %s", GetSQLValueString($colname_codigop, "int"));
...
$pdf->Cell(0,15,'                                    ORDEN DE COMPRA:  '.$row_Factura['codigoordcomp'],0,1);
...
$pdf->SetFont('Times','',9);
	...
	$pdf->Cell(0,5,'DIRECCION         :   '.ucfirst($row_Factura['direccion']),0,1);
	$pdf->Cell(0,5,'CIUDAD               :   '.ucfirst($row_Factura['ciudad']).'                            '.'TELEFONO: '.$row_Factura['celular'],0,1);
...
$codigoprod= $row_Listado_Productos['codigoprod'];
		...
		$pdf->Cell(19,5,$codigoprod,1);
      	$pdf->Cell(8,5,$cant,1);
	  	$pdf->Cell(135,5,strtoupper($descrip),1);
	...
	$pdf->Cell(30,6,'                                                                                                                                                                                   IGV 18%:    '.$row_Factura['igv'],0,1); 
