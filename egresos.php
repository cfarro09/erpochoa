...
<script>
    ...
    const dispose = async () => {
        ...
        datosplamar.closest(".divparent").style.display = "none";
        selectplamar.closest(".divparent").style.display = "none";
        $("#mdespose").modal()
        ...
        if (id != 11) {
            ...
            const query1 = `
                SELECT 
                    ...
                    IF(dd.tipo = 'ingreso', IF(dd.tipocliente = 'natural', (select cn.cedula from cnatural cn where cn.codigoclienten = dd.codigocliente), (select cj.ruc from cjuridico cj where cj.codigoclientej = dd.codigocliente)), '') extra,
                    dd.fromdespose
                FROM despose dd
                WHERE 
    ...
    const onloadPagoPlamar = async () => {
        ...
        cargarselect2("#selectplamar", ddd, "id", "descripcion", ["datoextra", "monto"]);
    }
