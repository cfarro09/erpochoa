...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "Ingresar")) {
  ...
  $insertSQL = sprintf("INSERT INTO serviciosaofrecer (codigosv, codigoclientej, codigoclienten, usuario_recepcion, personal_recepcion, fecha_recepcion, hora_recepcion, observacion_recepcion) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['codigosv'], "int"),
                       GetSQLValueString($_POST['codigoclientej'], "int"),
                       GetSQLValueString($_POST['codigoclienten'], "int"),
                       GetSQLValueString($_POST['usuario_recepcion'], "int"),
                       GetSQLValueString($_POST['personal_recepcion'], "int"),
                       GetSQLValueString($_POST['fecha_recepcion'], "date"),
                       GetSQLValueString($_POST['hora_recepcion'], "date"),
                       GetSQLValueString(strtoupper($_POST['observacion_recepcion']), "text"));

  mysql_select_db($database_Ventas, $Ventas);
  $Result1 = mysql_query($insertSQL, $Ventas) or die(mysql_error());

  $insertGoTo = "service_add.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<div class="input-group"><span id="sprytextfield1">
  <input type="text" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Persona Natural o Jur&iacute;dica" placeholder="Seleccionar Persona Natural o Jurídica" onkeypress="return cancelWrite(event)" onkeydown="return cancelWrite(event)" name="persona" id="persona" />
  ...
  <td colspan="2">
    <div class="col-md-10">
      <div class="form-group"><span id="spryselect1">
        <select name="codigosv" id="codigosv" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Servicio a Ofrecer">
...
<input type="text" class="form-control tooltips" data-placement="top" data-original-title="Fecha de Ingreso" id="fecha_recepcion" name="fecha_recepcion" onKeyPress="return cancelWrite(event)" onKeyDown="return cancelWrite(event)" value="<?php echo date('Y-m-d');?>"/>
...
<div class="form-group col-md-6">
<div class="input-group input-group-lg">
<input type="text" class="form-control tooltips" data-placement="top" data-original-title="Hora de Ingreso" id="hora_recepcion" name="hora_recepcion" onKeyPress="return cancelWrite(event)" onKeyDown="return cancelWrite(event)" 
...
<tr>
  ...
  <textarea name="observacion_recepcion" id="observacion_recepcion" rows="3" class="form-control tooltips" data-placement="top" data-original-title="Agregar Descripci&oacute;n u Observaci&oacute;n" placeholder="Descripción u Observación"></textarea>
...
<input name="usuario_recepcion" type="hidden" value="<?php echo $_SESSION['kt_login_id']; ?>" />
<input name="personal_recepcion" type="hidden" value="<?php echo $_SESSION['kt_codigopersonal'];
