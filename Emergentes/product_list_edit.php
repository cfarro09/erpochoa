...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "Ingresar")) {
	...
	if (!isset($_FILES['imagegordoputo']['tmp_name'])) {
		echo "";
		
	} else {
		
		
		$file = $_FILES['imagegordoputo']['tmp_name'];
		$image = addslashes(file_get_contents($_FILES['imagegordoputo']['tmp_name']));
		$image_name = addslashes($_FILES['imagegordoputo']['name']);
		$image_size = getimagesize($_FILES['imagegordoputo']['tmp_name']);
		...
		if ($image_size == FALSE) {
			...
			$ext = pathinfo($_FILES['imagegordoputo']['name'], PATHINFO_EXTENSION);
			$ext = strtolower($ext);
			move_uploaded_file($_FILES["imagegordoputo"]["tmp_name"], "../reservation/img/products/" . $colname_Producto . "." . $ext);

			$location = $colname_Producto . "." . $ext;

			//$pname = $_POST['codigoprod'];
			//mysql_select_db($database_Ventas, $Ventas);
			// $update = mysql_query("INSERT INTO producto_imagen (codigoprod, foto) VALUES ('$pname','$location')", $Ventas);
			//mysql_query("INSERT INTO producto_imagen (codigoprod, foto) VALUES ($pname,'$location')", $Ventas) or die(mysql_error());
	...
	if($location==NULL)
		$updateSQL = sprintf(
			"UPDATE producto SET nombre_producto=%s, codigomarca=%s, codigocat=%s, codigosubcat=%s, codigocolor=%s, codigopresent=%s, descripcion=%s, minicodigo=%s, codigo2=%s, codigo3=%s, validacion=%s WHERE codigoprod=%s",
			...
			$updateSQL = sprintf(
		"UPDATE producto SET nombre_producto=%s, img=%s, codigomarca=%s, codigocat=%s, codigosubcat=%s, codigocolor=%s, codigopresent=%s, descripcion=%s, minicodigo=%s, codigo2=%s, codigo3=%s, validacion=%s WHERE codigoprod=%s",
		GetSQLValueString(strtoupper($_POST['nombre_producto']), "text"),
...
<form action="<?php echo $editFormAction; ?>" method="POST" enctype='multipart/form-data'>
	...
	<?php
		echo '<img src="../reservation/img/products/' . $row_Producto['img'] . '" alt="' . $row_Producto['nombre_producto'] . '" title="' . $row_Producto['nombre_producto'] . '" width="150" height="150" /></a>
  </a>';
	

	?>
	<input type="file" name="imagegordoputo" accept="image/*">
				...
				<div class="col-md-8">
					<div class="form-group"><span id="spryselect1">
							<select name="codigocat" id="codigocat" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Categor&iacute;a">
								<option value="" <?php if (!(strcmp("", $row_Producto['codigocat']))) {
														echo "selected=\"selected\"";
													} ?>>-- Categor&iacute;as --</option>
								<?php
								do {
								?>
									<option value="<?php echo $row_Categorias['codigocat'] ?>" <?php if (!(strcmp($row_Categorias['codigocat'], $row_Producto['codigocat']))) {
																									echo "selected=\"selected\"";
							...
							<div class="col-md-10">
								<div class="form-group"><span id="spryselect3">
										<select name="codigomarca" id="codigomarca" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Marca">
											<option value="" <?php if (!(strcmp("", $row_Producto['codigomarca']))) {
																	echo "selected=\"selected\"";
																} ?>>-- Marca --</option>
											<?php
											do {
											?>
												<option value="<?php echo $row_Marca['codigomarca'] ?>" <?php if (!(strcmp($row_Marca['codigomarca'], $row_Producto['codigomarca']))) {
																											echo "selected=\"selected\"";
																										} ?>><?php echo $row_Marca['nombre'] ?></option>
											<?php
							...
							<div class="col-md-10">
								<div class="form-group"><span id="spryselect4">
										<select name="codigopresent" id="codigopresent" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Presentaci&oacute;n">
											<option value="" <?php if (!(strcmp("", $row_Producto['codigopresent']))) {
																	echo "selected=\"selected\"";
																} ?>>-- Presentaci&oacute;n --</option>
											<?php
											do {
											?>
												<option value="<?php echo $row_Presentacion['codigopresent'] ?>" <?php if (!(strcmp($row_Presentacion['codigopresent'], $row_Producto['codigopresent']))) {
																														echo "selected=\"selected\"";
																													} ?>><?php echo $row_Presentacion['nombre_presentacion'] ?></option>
											<?php
							...
							<div class="col-md-10">
								<div class="form-group"><span id="spryselect5">
										<select name="codigocolor" id="codigocolor" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Color">
											<option value="" <?php if (!(strcmp("", $row_Producto['codigocolor']))) {
																	echo "selected=\"selected\"";
																} ?>>-- Color --</option>
											<?php
											do {
											?>
												<option value="<?php echo $row_Colores['codigocolor'] ?>" <?php if (!(strcmp($row_Colores['codigocolor'], $row_Producto['codigocolor']))) {
																												echo "selected=\"selected\"";
																											} ?>><?php echo $row_Colores['nombre_color'] ?></option>
											<?php
