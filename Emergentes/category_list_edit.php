...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "Ingresar")) {
  $updateSQL = sprintf("UPDATE categoria SET nombre=%s WHERE codigocat=%s",
  ...
  $updateGoTo = "category_list_edit.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
if (isset($_GET['codigocat'])) {
  $colname_Editar = $_GET['codigocat'];
}
mysql_select_db($database_Ventas, $Ventas);
$query_Editar = sprintf("SELECT * FROM categoria WHERE codigocat = %s", GetSQLValueString($colname_Editar, "int"));
...
<input name="nombre" type="text" class="form-control tooltips" data-placement="top" data-original-title="Editar Nombre Categor&iacute;a" id="nombre" placeholder="Categoria" value="<?php echo $row_Editar['nombre']; ?>" maxlength="200"/>
<span class="textfieldRequiredMsg"></span><span class="textfieldMinCharsMsg"></span></span><span class="input-group-addon">
<i class="fa fa-th-large  font-blue-soft"></i>
</span>
</div>
</div>
</div>
<input name="codigocat" type="hidden" id="codigocat" value="<?php echo $row_Editar['codigocat']; ?>" />
</td>
</tr>
