...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "Ingresar")) {
  ...
  $updateSQL = sprintf("UPDATE historial_producto hp inner join (select max(codigohistprod) mcodigohistprod from historial_producto group by codigoprod) hp1 ON hp.codigohistprod=hp1.mcodigohistprod SET codigoprod=%s, precio_compra=%s, precio_venta=%s, cantidad=%s, codigoproveedor=%s, detalle_producto=%s, fecha=%s WHERE codigoprod=%s",
...
mysql_select_db($database_Ventas, $Ventas);
$query_Historialp = "SELECT * FROM historial_producto";
$historial_producto = mysql_query($query_Historialp, $Ventas) or die(mysql_error());
$row_historial_producto = mysql_fetch_assoc($historial_producto);
$totalRows_historial_producto = mysql_num_rows($historial_producto);
...
mysql_select_db($database_Ventas, $Ventas);
$query_Producto = sprintf("SELECT * FROM producto p inner join historial_producto hp ON  p.codigoprod=hp.codigoprod inner join proveedor pv on pv.codigoproveedor=hp.codigoproveedor WHERE p.codigoprod = %s order by hp.codigohistprod desc", GetSQLValueString($colname_Producto, "int"));
...
<input name="precio_compra" type="text" class="form-control tooltips" data-placement="top" data-original-title="Editar Precio de Compra" id="precio_venta" placeholder="Precio Compra" value="<?php echo $row_Producto['precio_compra']; ?>" maxlength="6"  />
...
<input name="precio_venta" type="text" class="form-control tooltips" data-placement="top" data-original-title="Editar Precio de Venta" id="precio_venta" placeholder="Precio Venta" value="<?php echo $row_Producto['precio_venta']; ?>" maxlength="6"  />
<span class="textfieldRequiredMsg"></span><span class="textfieldMinCharsMsg"></span></span><span class="input-group-addon"> 
<i class="fa fa-dollar font-blue-soft"></i> </span> </div>
</div>
</div>
  ...
  <tr>
    <td>
    <div class="col-md-8">
<div class="form-group"><span id="spryselect1"><input name="cantidad" type="text" class="form-control tooltips" data-placement="top" data-original-title="insertar cantidad de Compra" id="ruc2" placeholder="Cantidad Producto" value="<?php echo $row_Producto['cantidad']; ?>" maxlength="6" readonly  /><span class="selectInvalidMsg"></span><span class="selectRequiredMsg"></span></span></div>
</div>
    </td>
    <td>
    <div class="col-md-8">
...
<select name="codigoproveedor" id="codigoproveedor" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Proveedor" disabled>
  <option value=" <?php if (!(strcmp("", $row_Producto['codigoproveedor']))) {echo "selected=\"selected\"";} ?>">-- Proveedor --</option>
  <?php
do {  
?>
<option value="<?php echo $row_Proveedor['codigoproveedor']?>"<?php if (!(strcmp($row_Proveedor['codigoproveedor'], $row_Producto['codigoproveedor']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Proveedor['razonsocial']?></option>
  <?php
...
<select name="comprobante" id="comprobante" class="form-control tooltips" data-placement="top" data-original-title="Seleccionar Tipo Comprobante" disabled>
  <option value=" <?php if (!(strcmp("", $row_Producto['comprobante']))) {echo "selected=\"selected\"";} ?>">-- Comprobante --</option>
  <?php
do {  
?>
<option value="<?php echo $row_Producto['comprobante']?>"<?php if (!(strcmp($row_Producto['comprobante'], $row_Producto['comprobante']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Producto['comprobante']?></option>
  <?php
} while ($row_Proveedor = mysql_fetch_assoc($Proveedor));
  $rows = mysql_num_rows($Proveedor);
  if($rows > 0) {
...
<span class="selectInvalidMsg"></span><span class="selectRequiredMsg"></span></span></div>
    ...
    <td>
    <div class="col-md-8">
<div class="form-group"><span id="spryselect1"><input name="numero" type="text" class="form-control tooltips" data-placement="top" data-original-title="insertar numero de Comprobante" id="comprobante" placeholder="Numero Comprobante" value="<?php echo $row_Producto['numero']; ?>" maxlength="6" disabled  /><span class="selectInvalidMsg"></span><span class="selectRequiredMsg"></span></span></div>
</div>
    </td>
...
<span class="selectInvalidMsg"></span><span class="selectRequiredMsg"></span></span></div>
  ...
  <textarea name="detalle_producto" id="detalle_producto" rows="3" class="form-control tooltips" data-placement="top" data-original-title="Editar Otra Descripci&oacute;n y/o Caracter&iacute;stica del Producto" placeholder="Otra descripci&oacute;n y/o caracter&iacute;stica del producto"><?php echo $row_Producto['detalle_producto']; ?></textarea>
