...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "Ingresar")) {
  $updateSQL = sprintf("UPDATE color SET nombre_color=%s WHERE codigocolor=%s",
                       GetSQLValueString(strtoupper($_POST['nombre_color']), "text"),
                       GetSQLValueString($_POST['codigocolor'], "int"));

  mysql_select_db($database_Ventas, $Ventas);
  $Result1 = mysql_query($updateSQL, $Ventas) or die(mysql_error());

  $updateGoTo = "color_list_edit.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_Editar = "-1";
if (isset($_GET['codigocolor'])) {
  $colname_Editar = $_GET['codigocolor'];
}
mysql_select_db($database_Ventas, $Ventas);
$query_Editar = sprintf("SELECT * FROM color WHERE codigocolor = %s", GetSQLValueString($colname_Editar, "int"));
...
<input name="nombre_color" type="text" class="form-control tooltips"  data-placement="top" data-original-title="Editar Nombre de Color" id="nombre_color" placeholder="Color" value="<?php echo $row_Editar['nombre_color']; ?>" maxlength="100"  />
...
<input name="codigocolor" type="hidden" id="codigocolor" value="<?php echo $row_Editar['codigocolor']; ?>" />
</td>
</tr>
