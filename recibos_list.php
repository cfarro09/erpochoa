...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "Anular_Factura")) {
  $updateSQL = sprintf("UPDATE reciboventas set estadofact=0 WHERE codigorecibo=%s",
...
mysql_select_db($database_Ventas, $Ventas);
$query_Listado = "SELECT v.codigorecibo, v.codigo, v.tipocomprobante, v.fecha_emision, dv.codcomprobante, v.codigoventa, count(dv.codcomprobante) as cant_item, sum(dv.cantidad) as cant_articulo, v.total, v.igv, v.subtotal, CONCAT (cn.paterno,' ', cn.materno, ' ', cn.nombre) AS ClienteN, cj.razonsocial AS ClienteJ, CONCAT (p.paterno,' ', p.materno, ' ', p.nombre) AS Vendedor, v.codigoventa, v.estadofact FROM detalle_ventas dv inner join reciboventas v on dv.codcomprobante=v.codigoventa left join cnatural cn on v.codigoclienten=cn.codigoclienten left join cjuridico cj on v.codigoclientej=cj.codigoclientej inner join personal p on p.codigopersonal=v.codigopersonal where dv.codcomprobante!='' and (v.tipocomprobante='rec') group by dv.codcomprobante Order by v.fecha_emision DESC";
//$query_Listado = "SELECT v.codigoventas, v.codigo, v.tipocomprobante,  v.fecha_emision, dv.codcomprobante, v.codigoventa, count(dv.codcomprobante) as cant_item, sum(dv.cantidad) as cant_articulo, v.total, v.igv, v.subtotal, CONCAT (cn.paterno,' ', cn.materno, ' ', cn.nombre) AS ClienteN, cj.razonsocial AS ClienteJ FROM detalle_ventas dv inner join ventas v on dv.codcomprobante=v.codigoventa left join cnatural cn on v.codigoclienten=cn.codigoclienten left join cjuridico cj on v.codigoclientej=cj.codigoclientej where dv.codcomprobante!='' group by dv.codcomprobante Order by v.codigoventas DESC";
    ...
    <tbody>
      <?php do { ?>
        <tr>
          ...
          <td> <?php echo $i; ?> </td>
          <td align="center"><?php echo $row_Listado['tipocomprobante'].substr($row_Listado['codigorecibo'], -4); ?>               </td>
...
<td> <?php echo "&#36; ".$row_Listado['total']; ?></td>
         ...
         if ($row_Listado['estadofact'] == 1) { ?>
          <a class="btn yellow-crusta tooltips" data-placement="top" data-original-title="Imprimir Comprobante" href="Imprimir/recibo_ventas.php?codigoventas=<?php echo $row_Listado['codigorecibo']; ?>&codigo=<?php echo $row_Listado['codigo']; ?>&tipocliente=<?php echo $tipocliente; ?>" target="new"><i class="glyphicon glyphicon-credit-card" ></i></a>          </td>
